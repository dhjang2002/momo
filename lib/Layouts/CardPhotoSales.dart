...
class FileData {
  ...
  List<Files> flist = <Files>[];
  FileData({required this.photo_id});
}

class CardPhotoSales extends StatefulWidget {
  final int max_count;
  final String photo_type;
  final List<String> photo_ids;
  final String users_id;
  final String message;
  List<String>? title;
  final Function(int count, int index, String thumnails) onChanged;

  CardPhotoSales(
      {Key? key,
      required this.max_count,
      required this.photo_type,
      required this.photo_ids,
  ...
  @override
  _CardPhotoSalesState createState() => _CardPhotoSalesState();
}

class _CardPhotoSalesState extends State<CardPhotoSales> {
  final scaffoldKey = GlobalKey<ScaffoldState>();
  bool bReady = false;
  bool bLoaded = false;
  bool bWait = false;
  final List<FileData> _fdList = <FileData>[];
  ...
  void initState() {
    super.initState();
    for (var element in widget.photo_ids) {
      _fdList.add(FileData(photo_id: element.toString()));
    }
  ...
  Widget BuildItem(int index) {
    String url = URL_IMAGE + _fileList[index].url.toString();
  ...
  Future<void> _reload(bool bAct) async {
    setState(() {
      bLoaded = false;
    });

    for(int index=0; index<_fdList.length; index++) {
      String photo_id = _fdList[index].photo_id;
      await Remote.getFiles(
          ...
          onResponse: (List<Files> list) {
            setState(() {
              ...
              _fdList[index].flist = list;

              if(bAct) {
                String thumnails = "";
                for (var element in _fdList[index].flist) {
                  if (thumnails.isNotEmpty) {
                    thumnails += ";";
                  }
                  thumnails += element.thum_url!;
                  widget.onChanged(
                      _fdList[index].flist.length, index, thumnails);
                }
              }
            });
          });
  ...
  Future<void> _delete(Files info) async {
    for(int index=0; index<_fdList.length; index++) {
      String id = _fdList[index].photo_id;
    ...
    for (int index = 0; index < _fdList.length; index++) {
      await Remote.addFiles(
          ...
          params: {
            ...
            "photo_type": widget.photo_type,    // 'sales'
            "photo_id":   _fdList[index].photo_id,
          },
  ...
  Future <void> fromGallery() async {
    ...
    if (pick != null) {
      ...
      showToastMessage("사용할 수 없는 사진입니다.");
    }
  }
}
